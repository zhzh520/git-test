...
# 修复路径转义问题（原始字符串）
file_path = r"D:\Users\huihu\Desktop\2026\damoxing\W2V\hongloumeng.txt"
# 调用函数获取分词后的句子列表
...
if not sentences:
    ...
    # 保存模型
    model.save("honglou_w2v.model")
    print("模型已保存为 honglou_w2v.model")

    # 加载训练好的模型
    loaded_model = Word2Vec.load("honglou_w2v.model")

    # 找出与“贾宝玉”最相似的词
    try:
        similar_words = loaded_model.wv.most_similar('贾宝玉', topn=10)
        print("\n=== 与“贾宝玉”语义最相似的词 ===")
        for word, similarity in similar_words:
            print(f"{word}: {similarity:.4f}")
    except KeyError:
        print("\n错误：“贾宝玉”未出现在训练词典中，请检查文本是否包含该词！")
    #
    # try:
    #     similar_words = loaded_model.wv.most_similar('王熙凤', topn=10)
    #     print("\n=== 与“王熙凤”语义最相似的词 ===")
    #     for word, similarity in similar_words:
    #         print(f"{word}: {similarity:.4f}")
    # except KeyError:
    #     print("\n错误：“王熙凤”未出现在训练词典中，请检查文本是否包含该词！")

    # 类比推理：修正逻辑（贾宝玉 - 王熙凤 + 林黛玉）
    try:
        analogy_words = loaded_model.wv.most_similar(
            positive=['贾宝玉', '林黛玉'],  # 加的词
            negative=['王熙凤'],  # 减的词
            topn=10
        )
        print("\n=== 类比推理：贾宝玉 - 王熙凤 + 林黛玉 ===")
